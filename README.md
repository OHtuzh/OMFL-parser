...
Ваша задача состоит в том, что бы реализовать парсер для OMFL

## Формат

OMFL - файл это текстовый файл в кодировке ASCII формате OMFL. Для будет описана спецификация формата. Основной конструкцией для OMFL является пара KeyValue.
Формат чувствителен к регистру. Пробелы игнорируются.
...
Т.к. название секции это Ключ, то оно может содержать '.'. Что определяет вложенные друг в друга секции.
